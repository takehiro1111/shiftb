...
export const formatPersonIntroduction = (person: Person) => {
...
const even = (array: number[]) => array.filter((element) => element % 2 === 0);
...
const duplicateRemoval = (array: number[]) =>
...
const doubleArray = (array: number[]) => array.map((element) => element * 2);
...
//     第二引数: 30
//     返り値: { name: '次郎', age: 30 }
const ageJudge = (persons: Person[], age: number) =>
  persons.find((person) => person.age === age);
